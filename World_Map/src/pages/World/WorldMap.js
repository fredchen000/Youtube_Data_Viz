...
class WorldMap extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState){
    const {month, category, property, color } = nextProps;
    ...
    _.each( data, (item, key) => {
      ...
      if(k[0] === month && k[1] === category){
        item["toxic_rate"] = item["dislikes"] / item["views"];
        item["favor_rate"] = item["likes"] / item["views"];
        item["comment_rate"] = item["comment_count"] / item["views"];
        console.log(item);
  ...
  componentDidMount(){
    const {month, category, property, color } = this.props
    const svg = select(this.svgRef.current);
  ...
  render() {
    return (
        <div ref={this.wrapRef} className={styles.wrapDiv}>
